...
class GrabCut:
    def __init__(self, img, mask, rect=None, gmm_components=5):
        ...
        self.upright_V = np.empty((self.rows - 1, self.cols - 1)) # self.cols + 1
    ...
    def calc_beta_smoothness(self):
        ...
        self.upright_V = self.gamma / np.sqrt(2) * np.exp(-self.beta * np.sum(
            np.square(_upright_diff), axis=2)) # axis=2 and np.sqrt(2)
    ...
    def construct_gc_graph(self):
        ...
        # assert len(edges) == len(self.gc_graph_capacity)
        edges.extend(
            list(zip(pr_indexes[0], [self.gc_sink] * pr_indexes[0].size)))
        _D = -np.log(self.fgd_gmm.calc_prob(self.img.reshape(-1, 3)[pr_indexes]))
        self.gc_graph_capacity.extend(_D.tolist())
        assert len(edges) == len(self.gc_graph_capacity)
        ...
        # self.gc_graph_capacity.extend([9 * self.gamma] * bgd_indexes[0].size) # 9 * self.gamma
        # assert len(edges) == len(self.gc_graph_capacity)
        edges.extend(
            list(zip(bgd_indexes[0], [self.gc_sink] * bgd_indexes[0].size)))
        self.gc_graph_capacity.extend([9 * self.gamma] * bgd_indexes[0].size) # 9 * self.gamma
        assert len(edges) == len(self.gc_graph_capacity)

        edges.extend(
            list(zip([self.gc_source] * fgd_indexes[0].size, fgd_indexes[0])))
        self.gc_graph_capacity.extend([9 * self.gamma] * fgd_indexes[0].size) # 9 * self.gamma
        assert len(edges) == len(self.gc_graph_capacity)
        ...
        # assert len(edges) == len(self.gc_graph_capacity)
        edges.extend(
            list(zip(fgd_indexes[0], [self.gc_sink] * fgd_indexes[0].size)))
        self.gc_graph_capacity.extend([0] * fgd_indexes[0].size)
        assert len(edges) == len(self.gc_graph_capacity)
        ...
        for i, edge in enumerate(edges):
            self.edge_to_capacity[edge] = self.gc_graph_capacity[i]


    def st_mincut(self):
        bk = BoykovKolmogorov(self.edge_to_capacity, self.gc_source, self.gc_sink, self.rows, self.cols)
        belong, boundary = bk.max_flow()
        ...
        img_indexes = np.arange(self.rows * self.cols,
                                dtype=np.uint32).reshape(self.rows, self.cols)
        result[pr_indexes] = np.where(np.isin(img_indexes[pr_indexes], boundary),255, 0)
        cv.imwrite('./images/mask_with_cut.jpg', result)

        # print(belong)
        self.partition_obj = []
        self.partition_bkg = []
        for point in belong:
            if point[0] == self.gc_source:
                self.partition_obj.append(point[1])
            elif point[1] == self.gc_sink:
                self.partition_bkg.append(point[0])
        ...
        # self.partition_bkg = []
        # for point in mincut:
        #     if point[1] == self.gc_sink:
        #         self.partition_obj.append(point[0])
        #     elif point[0] == self.gc_source:
        #         self.partition_bkg.append(point[1])
    ...
    def estimate_segmentation(self):
        ...
        print('foreground pixels: %d, background pixels: %d' % (
            len(self.partition_obj), len(self.partition_bkg)))
        pr_indexes = np.where(np.logical_or(
            self.mask == DRAW_PR_BG['val'], self.mask == DRAW_PR_FG['val']))
        img_indexes = np.arange(self.rows * self.cols,
                                dtype=np.uint32).reshape(self.rows, self.cols)
        self.mask[pr_indexes] = np.where(np.isin(img_indexes[pr_indexes], self.partition_obj),
...
if __name__ == '__main__':
    ...
    while(1):
        ...
        if k == 27:         # esc to exit
            ...
            and again press 'n' \n""")
            print(rect)
            cv.imwrite('./images/input_with_rectangle.jpg', img)
            cv.imwrite('./images/input.jpg', img2)
        ...
        cv.imwrite('./images/mask.jpg', mask2)
        cv.imwrite('./images/mask_with_foreground.jpg', output)
